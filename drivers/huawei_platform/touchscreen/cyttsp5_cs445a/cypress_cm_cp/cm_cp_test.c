...
#if 0
static int retrieve_button_raw_data(uint16_t read_offset, uint16_t read_length,
        int32_t *raw_data)
{
    uint16_t actual_read_length;
    uint8_t data_format;
    ...
    data = kzalloc(read_length * BUTTONS_NUM_ELEMENTS * sizeof(uint32_t), GFP_KERNEL);
    if (!data)
       return -ENOMEM;

    ret = retrieve_panel_scan(read_offset * BUTTONS_NUM_ELEMENTS,
        read_length * BUTTONS_NUM_ELEMENTS, BUTTONS_DATA_ID,
        &actual_read_length, &data_format, data);
    if (ret) {
        tp_log_err("%s:Unable to retrieve buttons raw data!\n",__func__);
        goto free;
    }

    if (read_length * BUTTONS_NUM_ELEMENTS != actual_read_length) {
        ret = -EINVAL;
        goto free;
    }
