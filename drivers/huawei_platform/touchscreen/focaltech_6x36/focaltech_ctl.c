...
static int ft_rw_iic_drv_major = 0;
...
{
	int ret = 0;	
	ret = ft6x06_i2c_Read(client, NULL, 0, buf, length);

	if(ret<0)
		tp_log_err("%s:IIC Read failed\n",
				__func__);
    	return ret;
}
...
{
	int ret = 0;
	ret = ft6x06_i2c_Write(client, buf, length);
	if(ret<0)
		tp_log_err("%s:IIC Write failed\n",
				__func__);
	return ret;
}
...
static void ft_rw_iic_drv_setup_cdev(struct ft_rw_i2c_dev *dev, int index)
{
	...
	if (err)
		tp_log_err("Error %d adding LED%d", err, index);
}
...
static int ft_rw_iic_drv_myinitdev(struct i2c_client *client)
{
	int err = 0;
	dev_t devno = 0;
	tp_log_info("in ft_rw_iic_drv_myinitdev\n");
	if (ft_rw_iic_drv_major)
	{
		devno = MKDEV(ft_rw_iic_drv_major, 0);
	...
	if (err < 0) {
		tp_log_err("%s:ft_rw_iic_drv failed  error code=%d---\n",
				__func__, err);
		return err;
	}
	...
	if (!ft_rw_i2c_dev_tt){
		...
		tp_log_err("%s:ft_rw_iic_drv failed\n",
				__func__);
		return err;
	}
	...
	if (IS_ERR(fts_class)) {
		tp_log_err("%s:failed in creating class.\n",
				__func__);
		return -1; 
	} 
	/*create device node*/
	device_create(fts_class, NULL, MKDEV(ft_rw_iic_drv_major, 0), 
			NULL, FT_RW_IIC_DRV);
	tp_log_info("out ft_rw_iic_drv_myinitdev\n");
	return 0;
}

int ft_rw_iic_drv_init(struct i2c_client *client)
{
	tp_log_info(" in ft_rw_iic_drv_init\n");
