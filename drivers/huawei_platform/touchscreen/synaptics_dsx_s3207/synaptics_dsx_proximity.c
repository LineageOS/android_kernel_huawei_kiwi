...
static void prox_finger_report(void)
{
	...
	if (retval < 0) {
		tp_log_err("%s: Failed to read proximity data registers\n",
				__func__);
		return;
	}
	...
	if (data->air_swipe_det) {
		tp_log_debug("%s: Swipe direction 0 = %d\n",
				__func__, data->air_swipe_dir_0);
		tp_log_debug("%s: Swipe direction 1 = %d\n",
				__func__, data->air_swipe_dir_1);
	}

	if (data->large_obj_det) {
		tp_log_debug("%s: Large object activity = %d\n",
				__func__, data->large_obj_act);
	}

	if (data->hover_pinch_det) {
		tp_log_debug("%s: Hover pinch direction = %d\n",
				__func__, data->hover_pinch_dir);
	}

	if (data->object_present) {
		tp_log_debug("%s: Object presence detected\n",
				__func__);
	}
...
static int prox_set_enables(void)
{
	...
	if (retval < 0) {
		tp_log_err("%s: Failed to write to proximity_enables\n",
				__func__);
		return retval;
	}
	...
	if (retval < 0) {
		tp_log_err("%s: Failed to write to general_control\n",
				__func__);
		return retval;
	}
...
static int prox_scan_pdt(void)
{
	...
	tp_log_err("%s: Failed to find F5\n",
			__func__);
	return -EINVAL;
...
f51_found:
	...
	if (retval < 0) {
		tp_log_err("%s: Failed to set interrupt enable bit\n",
				__func__);
		return retval;
	}
...
static int synaptics_rmi4_prox_init(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (!prox) {
		tp_log_err("%s: Failed to alloc mem for prox\n",
				__func__);
		retval = -ENOMEM;
		goto exit;
	}
	...
	if (!prox->finger_data) {
		tp_log_err("%s: Failed to alloc mem for finger_data\n",
				__func__);
		retval = -ENOMEM;
	...
	if (prox->prox_dev == NULL) {
		tp_log_err("%s: Failed to allocate proximity device\n",
				__func__);
		retval = -ENOMEM;
		goto exit_free_finger_data;
	}
	...
	if (retval) {
		tp_log_err("%s: Failed to register proximity device\n",
				__func__);
