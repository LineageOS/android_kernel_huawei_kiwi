		...
		char *buf, loff_t pos, size_t count)
{
	...
	if (length > (REG_ADDR_LIMIT - address)) {
		tp_log_err("%s: Out of register map limit\n",
				__func__);
		return -EINVAL;
	}
	...
	if (length) {
		retval = rmidev->fn_ptr->read(rmidev->rmi4_data,
				...
				length);
		if (retval < 0) {
			tp_log_err("%s: Failed to read data\n",
					__func__);
			return retval;
		}
	} else {
		...
		char *buf, loff_t pos, size_t count)
{
	...
	if (length) {
		retval = rmidev->fn_ptr->write(rmidev->rmi4_data,
				...
				length);
		if (retval < 0) {
			tp_log_err("%s: Failed to write data\n",
					__func__);
			return retval;
		}
	} else {
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	tp_log_debug("%s: Attention interrupt disabled\n",
			__func__);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	tp_log_debug("%s: Attention interrupt enabled\n",
			__func__);
...
static loff_t rmidev_llseek(struct file *filp, loff_t off, int whence)
{
	...
	if (newpos < 0 || newpos > REG_ADDR_LIMIT) {
		tp_log_err("%s: New position 0x%04x is invalid\n",
...
static void rmidev_device_cleanup(struct rmidev_data *dev_data)
{
	...
	if (dev_data) {
		...
		tp_log_debug("%s: rmidev device removed\n",
				__func__);
	}
...
static int rmidev_init_device(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (!rmidev) {
		tp_log_err("%s: Failed to alloc mem for rmidev\n",
				__func__);
		retval = -ENOMEM;
		goto err_rmidev;
	}
	...
	if (retval < 0) {
		tp_log_err("%s: Failed to create device class\n",
				__func__);
	...
	if (rmidev_major_num) {
		...
		if (retval < 0) {
			tp_log_err("%s: Failed to allocate char device region\n",
					__func__);
			goto err_device_region;
		}

		rmidev_major_num = MAJOR(dev_no);
		tp_log_debug("%s: Major number of rmidev = %d\n",
				__func__, rmidev_major_num);
	}

	dev_data = kzalloc(sizeof(*dev_data), GFP_KERNEL);
	if (!dev_data) {
		tp_log_err("%s: Failed to alloc mem for dev_data\n",
				__func__);
		retval = -ENOMEM;
		goto err_dev_data;
	}
	...
	if (retval < 0) {
		tp_log_err("%s: Failed to add rmi char device\n",
				__func__);
	...
	if (IS_ERR(device_ptr)) {
		tp_log_err("%s: Failed to create rmi char device\n",
				__func__);
		retval = -ENODEV;
		goto err_char_device;
	}
	...
	if (retval < 0) {
		tp_log_err("%s: Failed to export attention gpio\n",
				__func__);
	} else {
		retval = gpio_export_link(&(rmi4_data->input_dev->dev),
				"attn", rmi4_data->board->irq_gpio);
		if (retval < 0) {
			tp_log_err("%s Failed to create gpio symlink\n",
					__func__);
		} else {
			tp_log_debug("%s: Exported attention gpio %d\n",
