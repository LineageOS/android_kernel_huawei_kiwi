...
#define  MAX_PROCNAME_LENGTH  20
#define CMD_HEAD_LENGTH     (sizeof(st_cmd_head) - sizeof(u8*))
static char procname[MAX_PROCNAME_LENGTH] = {0};
...
static void tool_set_proc_name(char * procname)
{
    ...
    //tp_log_debug("compile date: %s", date);
    
    ret = sscanf(date, "%s %d %d", month, &n_day, &n_year);
    if(!ret)
    {
        tp_log_err("%s:sscanf return invalid,line=%d\n",__func__,__LINE__);
        return;
    }    
    for (i = 0; i < 12; ++i)
    {
    ...
    snprintf(procname, MAX_PROCNAME_LENGTH, "gmnode%04d%02d%02d", n_year, n_month, n_day);    
    //sprintf(procname, "goodix_tool");
    //tp_log_debug("procname = %s", procname);
}
...
static s32 tool_i2c_write_no_extra(u8* buf, u16 len)
{
    ...
    //  msg.len   = len;
    //  msg.buf   = buf;
	tp_log_debug("%s,begin, len =%d line = %d\n",__func__, len, __LINE__);
    while(pos != data_length)
    {
        if ((data_length - pos) > IIC_MAX_TRANSFER_SIZE)
        {
...
//        || !strncmp(IC_TYPE, "GT813", 5))
        ...
        && strncmp(IC_TYPE, "GTxxx", 5) && strncmp(IC_TYPE, "GT9XX", 5))
    {
        ...
        tp_log_debug("I2C function: with pre and end cmd!\n");
    }
    else
    {
        tool_i2c_read = tool_i2c_read_no_extra;
        tool_i2c_write = tool_i2c_write_no_extra;
        tp_log_info("I2C function: without pre and end cmd!\n");
    }
}
...
static void unregister_i2c_func(void)
{
    tool_i2c_read = NULL;
    tool_i2c_write = NULL;
    tp_log_info("I2C function: unregister i2c transfer function!\n");
}
...
s32 init_wr_node(struct i2c_client *client)
{
    ...
    if (i)
    {
        ...
        tp_log_info("%s,Applied memory size:%d.\n", __func__,DATA_LENGTH);
    }
    else
    {
        tp_log_err("%s,Apply for memory failed.\n", __func__);
        return FAIL;
    }
    ...
    {
        tp_log_err("Couldn't create proc entry!\n");
        return FAIL;
    }
    else
    {
        tp_log_info("%s,Create proc entry success!\n", __func__);
...
static u8 relation(u8 src, u8 dst, u8 rlt)
{
    ...
    case 1:
        ret = (src == dst) ? true : false;
        tp_log_debug("equal:src:0x%02x   dst:0x%02x   ret:%d.", src, dst, (s32)ret);
        break;
...
//    memcpy(buf, &cmd_head.flag_addr, cmd_head.addr_len);//Modified by Scott, 2012-02-17
    ...
    for (i = 0; i < cmd_head.times; i++)
    {
        ...
        {
            tp_log_err("Read flag data failed!");
            return FAIL;
        }
        if (true == relation(buf[GTP_ADDR_LENGTH], cmd_head.flag_val, cmd_head.flag_relation))
        {
            tp_log_debug("value at flag addr:0x%02x.", buf[GTP_ADDR_LENGTH]);
            tp_log_debug("flag value:0x%02x.", cmd_head.flag_val);
            break;
        }
    ...
    if (i >= cmd_head.times)
    {
        tp_log_err("Didn't get the flag to continue!");
        return FAIL;
    }
...
ssize_t goodix_tool_write(struct file *filp, const char __user *buff, size_t len, loff_t *off)
{
    ...
    tp_log_debug("[Operation]wr: %02X", cmd_head.wr);
    tp_log_debug("[Flag]flag: %02X, addr: %02X%02X, value: %02X, relation: %02X", cmd_head.flag, cmd_head.flag_addr[0], 
                        cmd_head.flag_addr[1], cmd_head.flag_val, cmd_head.flag_relation);
    tp_log_debug("[Retry]circle: %d, times: %d, retry: %d, delay: %d", (s32)cmd_head.circle, (s32)cmd_head.times, 
                        (s32)cmd_head.retry, (s32)cmd_head.delay);
    tp_log_debug("[Data]data len: %d, addr len: %d, addr: %02X%02X, buffer len: %d, data[0]: %02X", (s32)cmd_head.data_len, 
    ...
    if (1 == cmd_head.wr)
    {
        ...
        {
            tp_log_err("[WRITE]Write data failed!");
            return -EPERM;
        }
    ...
    else if(17 == cmd_head.wr)
    {
        ...
        if(ret)
        {
            tp_log_debug("copy_from_user failed.");
            return -EPERM;
        }
        if(cmd_head.data[GTP_ADDR_LENGTH])
        {
            tp_log_info("gtp enter rawdiff.");
            ts->gtp_rawdiff_mode = true;
        }
        else
        {
            ts->gtp_rawdiff_mode = false;
            tp_log_info("gtp leave rawdiff.");
        }
    }
#ifdef UPDATE_FUNCTIONS
    ...
    else if (15 == cmd_head.wr) //Update firmware!
    {
        ...
        if (FAIL == gup_update_proc(cmd_head.data))
        {
            return -EPERM;
        }
    }
...
ssize_t goodix_tool_read(struct file *file, char __user *page, size_t size, loff_t *ppos)
{
    ...
    {
        //tp_log_debug("[HEAD]wr: %d", cmd_head.wr);
        //tp_log_debug("[PARAM]size: %d, *ppos: %d", size, (int)*ppos);
        //tp_log_debug("[TOOL_READ]ADB call again, return it.");
        return 0;
    }
    ...
    else if (!cmd_head.wr)
    {
        ...
        tp_log_debug("[CMD HEAD DATA] ADDR:0x%02x%02x.", cmd_head.data[0], cmd_head.data[1]);
        tp_log_debug("[CMD HEAD ADDR] ADDR:0x%02x%02x.", cmd_head.addr[0], cmd_head.addr[1]);
        ...
        while(data_len > 0)
        {
            ...
            {
                tp_log_err("[READ]Read data failed!");
                return -EPERM;
            }
