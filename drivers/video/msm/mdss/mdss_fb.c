			     ...
			     struct device_attribute *attr,const char *buf, size_t count)
{
	...
	{
		pr_err("This panel maybe sleep ,or can not support set cabc mode\n");
		return -EINVAL;
	}
	return count;
}
		...
		struct device_attribute *attr, char *buf)
{
	...
	{
		pr_err("This panel maybe sleep or can not support lcd-frame-checkum!\n");
	}
...
void mdss_fb_set_backlight(struct msm_fb_data_type *mfd, u32 bkl_lvl)
{
	...
	/*cancle the esd esd delay work before set backlight */
		...
		if ( mfd->bl_level_scaled == 0 )
		{
			...
			if( is_first_on) {
				pr_info("%s:first panel on do not read 0x0A\n",__func__);
				is_first_on = false;
			}
			else {
