...
int mdss_dsi_panel_reset(struct mdss_panel_data *pdata, int enable)
{
	...
	if (enable) {
		...
		if(enable_PT_test && ctrl_pdata->reset_for_pt_flag)
		{
			...
			usleep( 20*1000);
			gpio_set_value((ctrl_pdata->rst_gpio), 0);
			pr_info("%s: panel pull reset low-high-low when suspend.\n",__func__);
		}
		gpio_free(ctrl_pdata->rst_gpio);
		if (gpio_is_valid(ctrl_pdata->mode_gpio))
...
static int mdss_dsi_panel_on(struct mdss_panel_data *pdata)
{
	...
	{
		pr_info("read from debug file and write to LCD!\n");
...
static int mdss_lcd_frame_checksum(struct mdss_panel_data *pdata)
{
	...
	if(!ctrl->frame_checksum_support){
		pr_info("%s:this panel frame checksum not supported!\n",__func__);
	...
	do{
		...
		if(i == 0 && rdata != ctrl->frame_crc_read_cmds_value[offset+i]){
			...
			if(rdata != ctrl->frame_crc_read_cmds_value[offset+i])
			{
				...
				{
					ret = -1;
					pr_err("%s:frame checksum fail! The %dth rdata = [0x%x]\n",__func__,i,rdata);
					break;
				}
			}
		}
			...
			struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	pr_info("%s: frame checksum %s by this panel!\n",__func__,
	...
	if(ctrl_pdata->frame_checksum_support)
	{
		...
		if(rc < 0)
		{
			pr_err("%s: parse frame_crc_enable command fail, frame_checksum not supported!\n",
					__func__);
			goto exit;
		}
		...
		if(rc < 0)
		{
			pr_err("%s: parse frame_crc_disable command fail, frame_checksum not supported!\n",
					__func__);
			goto exit;
		}
		...
		if ((!data) || (!len)) {
			pr_err("%s: parse frame_crc_read command fail, frame_checksum not supported!\n",
						__func__);
			goto exit;
		}
		else
		...
		if ((!data) || (!len)) {
			pr_err("%s: parse frame_crc_read command value fail, frame_checksum not supported!\n",
				__func__);
			goto exit;
		}
		else{
	...
	exit:
		...
		pr_err("%s : parse frame_crc dts cmds unsuccess! disable checksum!\n",__func__);
	return;
}
#endif
