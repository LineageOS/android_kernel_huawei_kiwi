...
======================================================================================*/
int hw_discharge_voltage_check(struct qpnp_bms_chip * chip, int soc, int batt_temp)
{
...
static void monitor_soc_work(struct work_struct *work)
{
...
#ifdef CONFIG_HUAWEI_KERNEL
			new_soc = hw_discharge_voltage_check(chip, new_soc, batt_temp);
	...
	if (chip->reported_soc_in_use && chip->charger_removed_since_full
		...
		if (chip->reported_soc_change_sec > ui_soc_catch_time) {
