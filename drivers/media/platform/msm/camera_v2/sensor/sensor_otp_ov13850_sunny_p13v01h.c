...
#define HW_CMR_LOG_TAG "sensor_otp_ov13850_sunny_p13v01h"
...
***************************************************************************/
static int32_t ov13850_sunny_p13v01h_cci_i2c_write(struct msm_sensor_ctrl_t *s_ctrl,uint32_t addr, u16 data)
{
	int32_t rc = -EFAULT;
...
***************************************************************************/
static int32_t ov13850_sunny_p13v01h_cci_i2c_read(struct msm_sensor_ctrl_t *s_ctrl, uint32_t addr, uint16_t *data)
{
	int32_t rc = -EFAULT;
...
static bool ov13850_read_otp(struct msm_sensor_ctrl_t *s_ctrl,uint16_t i2c_addr,u16 reg,u8 *buf,u16 count)
{
	...
	for (i=0; i<count; i++)
	{
		ret =ov13850_sunny_p13v01h_cci_i2c_read(s_ctrl,(reg+i) ,&val);
		if (ret !=0)
		{
...
static void ov13850_otp_set_awb(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	CMR_LOGD("ov13850_get_otp_from_sensor() ~ %s R_gain=0x%x, B_gain=0x%x, G_gain=0x%x\n",__func__, R_gain, B_gain, G_gain);
       /* awb set flag */
	ov13850_sunny_p13v01h_cci_i2c_read(s_ctrl,0x5001, &temp);
	temp = 0x02 | temp;
	ov13850_sunny_p13v01h_cci_i2c_write(s_ctrl,0x5001, temp);

	if (R_gain>0x400)
	{
		ov13850_sunny_p13v01h_cci_i2c_write(s_ctrl,0x5056, R_gain>>8);
		ov13850_sunny_p13v01h_cci_i2c_write(s_ctrl,0x5057, R_gain & 0x00ff);
	}

	if (G_gain>0x400)
	{
		ov13850_sunny_p13v01h_cci_i2c_write(s_ctrl,0x5058, G_gain>>8);
		ov13850_sunny_p13v01h_cci_i2c_write(s_ctrl,0x5059, G_gain & 0x00ff);
	}

	if (B_gain>0x400)
	{
		ov13850_sunny_p13v01h_cci_i2c_write(s_ctrl,0x505a, B_gain>>8);
		ov13850_sunny_p13v01h_cci_i2c_write(s_ctrl,0x505b, B_gain & 0x00ff);
	}
...
static void ov13850_otp_set_lsc(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	u16  temp = 0;
       ...
       /* lsc set flag */
	ov13850_sunny_p13v01h_cci_i2c_read(s_ctrl,0x5000, &temp);
	temp = 0x01 | temp;
	ov13850_sunny_p13v01h_cci_i2c_write(s_ctrl,0x5000, temp);

	for (i=0; i<OV13850_OTP_LSC_SIZE; i++)
	{
		ov13850_sunny_p13v01h_cci_i2c_write(s_ctrl,0x5200+i, ov13850_otp.lsc[i]);
	}
...
******************************************************************************/
int ov13850_sunny_p13v01h_otp_func(struct msm_sensor_ctrl_t * s_ctrl, int index)
{
	 int rc = 0;
	CMR_LOGD("%s enters!\n",__func__);
