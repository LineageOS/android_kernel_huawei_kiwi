...
//#define		INIT_FAST			// Initialize of continuous trans mode
#ifdef		INIT_FAST
...
// TDK 9.5/9.2ohm Actuator TVA-652 ***************************
...
#ifdef	USE_VH_SYNC
	RegWriteA( STROBEFC,	0x80 ) ;				// 0x001C	Strobe
...
#else
       CHECK_RETURN(StartUpGainContIni());
#endif
...
static int IniFil( void )
{
	...
	while( CsFilRamY[ UcAryId ] != 0xFF )
	{
		...
		CntWrtRag( ( unsigned char * )&CsFilRamYDat[ UsDatId ], UsDatNum ) ;
		UsDatId	+= UsDatNum ;
		UcAryId++ ;
	}	
#else
...
int imx278_liteon_ois_change_mode(uint32_t mode)
{
	if(mode == OIS_PREVIEW)
	{
...
int imx278_liteon_ois_turn_onoff(uint32_t on)
{
    if(on)
    {
    	CHECK_RETURN(imx278_liteon_ois_change_mode(OIS_CAPTURE));
        CHECK_RETURN(OisEna());
    }
    else
    {
    	CHECK_RETURN(imx278_liteon_ois_change_mode(OIS_PREVIEW));
        CHECK_RETURN(RtnCen(0x00));
    }
...
//for test interface
int imx278_liteon_ois_AF_SetPos(short data)
{
    CHECK_RETURN(AF_SetPos(data));
    return 0;
}
int imx278_liteon_ois_RamAccFixMod(uint8_t data)
{
    CHECK_RETURN(RamAccFixMod(data));
    return 0;
}
int imx278_liteon_ois_RtnCen(uint8_t data)
{
    return RtnCen(data);
}
int imx278_liteon_ois_SetPanTiltMode(uint8_t data)
{
