...
struct ov5648_sunny_p5v18g_otp_struct {
	...
	uint16_t vcm_start;// vcm start current
	uint16_t vcm_end;// vcm end current
};

//R/G and B/G value of Golden Samples.
static  uint32_t rg_ratio_typical = 0x2eb;//the average of 4 Golden samples' RG ratio
static  uint32_t bg_ratio_typical = 0x2dc;//the average of 4 Golden samples' BG ratio
...
static  uint32_t ov5648_sunny_p5v18g_otp_read_flag = 0 ;

//OV5648 opt has 2 bank
#define OV5648_SUNNY_P5V18G_OTP_BANK0    0
#define OV5648_SUNNY_P5V18G_OTP_BANK1    1

//bit0:module info, bit1:AWB, bit2:VCM
#define OV5648_SUNNY_P5V18G_OTP_MODULE_INFO          1
#define OV5648_SUNNY_P5V18G_OTP_AWB                  2
#define OV5648_SUNNY_P5V18G_OTP_VCM                  4
#define OV5648_SUNNY_P5V18G_OTP_ALL  (OV5648_SUNNY_P5V18G_OTP_MODULE_INFO | OV5648_SUNNY_P5V18G_OTP_AWB | OV5648_SUNNY_P5V18G_OTP_VCM)
...
***************************************************************************/
static int32_t ov5648_sunny_p5v18g_cci_i2c_read(struct msm_sensor_ctrl_t *s_ctrl, uint32_t addr, uint16_t *data)
{
	int32_t rc = -EFAULT;
...
/******************************************************************************
Function   :    ov5648_sunny_p5v18g_otp_clear
Description:    Disable  OTP read and clear otp buffers, must called wehen read anothe bank.
******************************************************************************/
void ov5648_sunny_p5v18g_otp_clear(struct msm_sensor_ctrl_t * s_ctrl)
{
	int i = 0;
	//Disable OTP read
	ov5648_sunny_p5v18g_cci_i2c_write(s_ctrl, 0x3d81, 0x00);

	for ( i = 0; i < 16; i++)//16 regs in on bank
	{
		//Clear otp buffers
		ov5648_sunny_p5v18g_cci_i2c_write(s_ctrl, 0x3d00 + i, 0x00);//0x3d00~0x3d0f
	}
...
static int ov5648_sunny_p5v18g_update_otp(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	memset(&current_otp, 0, sizeof(struct ov5648_sunny_p5v18g_otp_struct));

	// R/G and B/G of current camera module is read out from sensor OTP
	ov5648_sunny_p5v18g_read_otp(s_ctrl, &current_otp);
														...
														uint32_t b_gain)
{
	...
	if (g_gain > 0x400) {
		ov5648_sunny_p5v18g_cci_i2c_write(s_ctrl, 0x5188, g_gain >> 8);
		ov5648_sunny_p5v18g_cci_i2c_write(s_ctrl, 0x5189, g_gain & 0x00ff);
	}

	if (b_gain > 0x400) {
		ov5648_sunny_p5v18g_cci_i2c_write(s_ctrl, 0x518a, b_gain >> 8);
		ov5648_sunny_p5v18g_cci_i2c_write(s_ctrl, 0x518b, b_gain & 0x00ff);
	}
...
/******************************************************************************
Function   :  ov5648_sunny_p5v18g_otp_func
Description:  read the otp info
******************************************************************************/
int ov5648_sunny_p5v18g_otp_func(struct msm_sensor_ctrl_t * s_ctrl, int index)
{
	...
	if (0 == (ov5648_sunny_p5v18g_otp_read_flag & OV5648_SUNNY_P5V18G_OTP_AWB))
	{
		if(0 != ov5648_sunny_p5v18g_update_otp(s_ctrl))
		{
	...
	ov5648_sunny_p5v18g_update_awb_gain(s_ctrl, gr_gain, gg_gain, gb_gain);
