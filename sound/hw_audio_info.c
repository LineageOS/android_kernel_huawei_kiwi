...
static ssize_t audio_log_level_show(struct device_driver *driver, char *buf)
{
    ...
    if(NULL == buf)
    {
        pr_err("%s get NULL argument/n", __func__);
        return 0;
    }
    ...
    switch(audio_log_level)
    {
        ...
        default:
            level_char = ' ';
            pr_err("Unkown audio log level/n");
            break;
    }

    pr_info("Current log level is %d:%c/n", audio_log_level, level_char);

    /* Those logs are for log level confirm and test */
    pr_verbose("This is verbose log/n");
    pr_debug("This is debug log/n");
    pr_info("This is info log/n");
    pr_info("This is notice log/n");
    pr_warn("This is warning log/n");
    pr_err("This is error log/n");
...
static ssize_t audio_log_level_store(struct device_driver *driver, const char *buf, size_t count)
{
    ...
    if((NULL == buf) || (count < 1))
    {
        pr_err("%s get illegal arguments/n", __func__);
        return -EINVAL;
    }

    level_char = buf[0];
    pr_info("Get audio log level char %c/n", level_char);
    ...
    switch(level_char)
    {
        ...
        case '0':
            ...
            break;
        default:
            pr_err("Unkown audio log level character: %c/n", level_char);
    ...
    {
        pr_info("Change audio log level from %d to %d/n", audio_log_level, new_log_level);
        audio_log_level = new_log_level;
    }
    else
    {
        pr_info("Audio log level remains %d/n", audio_log_level);
    }
    return count;
}
