...
static int32_t afe_callback(struct apr_client_data *data, void *priv)
{
	...
	if (data->opcode == RESET_EVENTS) {
		ad_logd("%s: reset event = %d %d apr[%pK]\n",
			__func__,
	...
	if (data->opcode == AFE_PORT_CMDRSP_GET_PARAM_V2) {
		...
		if ((data->payload_size < sizeof(this_afe.calib_data))
			...
			ad_loge("%s: Error: size %d payload %pK token %d\n",
...
static int afe_send_cal_block(u16 port_id, struct cal_block_data *cal_block)
{
	...
	ad_logd("%s: AFE cal sent for device port = 0x%x, cal size = %zd, cal addr = 0x%pK\n",
		__func__, port_id,
...
static void remap_cal_data(struct cal_block_data *cal_block, int cal_index)
{
	...
	if ((cal_block->map_data.map_size > 0) &&
		...
		if (ret < 0) {
			...
			ad_logd("%s: mmap did not work! addr = 0x%pK, size = %zd\n",
				__func__,
...
int afe_cmd_memory_map(phys_addr_t dma_addr_p, u32 dma_buf_sz)
{
	...
	ad_logd("%s: dma_addr_p 0x%pK , size %d\n", __func__,
			...
			struct afe_audio_client *ac)
{
	...
	if (port->buf[0].data) {
		ad_logd("%s: data[%pK]phys[%pK][%pK] , client[%pK] handle[%pK]\n",
			__func__,
			port->buf[0].data,
			&port->buf[0].phys,
			&port->buf[0].phys,
...
int afe_map_rtac_block(struct rtac_cal_block_data *cal_block)
{
	...
	result = afe_cmd_memory_map(cal_block->cal_data.paddr,
		cal_block->map_data.map_size);
	if (result < 0) {
		ad_loge("%s: afe_cmd_memory_map failed for addr = 0x%pK, size = %d, err %d\n",
