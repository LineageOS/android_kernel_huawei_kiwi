		      ...
		      void *data, int size)
{
	...
	if (size <= 0 || !data) {
		ad_loge("DTS_EAGLE_ADM - %s: invalid size %i or pointer %pK.\n",
			__func__, size, data);
		return -EINVAL;
	}
...
static int32_t adm_callback(struct apr_client_data *data, void *priv)
{
	...
	if (data->opcode == RESET_EVENTS) {
		ad_logd("%s: Reset event is received: %d %d apr[%pK]\n",
			__func__,
...
static void remap_cal_data(struct cal_block_data *cal_block, int cal_index)
{
	...
	if ((cal_block->map_data.map_size > 0) &&
		...
		if (ret < 0) {
			...
			ad_logd("%s: ADM mmap did not work! addr = 0x%pK, size = %zd ret %d\n",
				__func__,
...
static void send_adm_custom_topology(void)
{
	...
	atomic_set(&this_adm.adm_stat, 0);
	ad_logd("%s: Sending ADM_CMD_ADD_TOPOLOGIES payload = 0x%pK, size = %d\n",
			      ...
			      int app_type, int acdb_id, int sample_rate)
{
	...
	atomic_set(&this_adm.copp.stat[port_idx][copp_idx], 0);
	ad_logd("%s: Sending SET_PARAMS payload = 0x%pK, size = %d\n",
		__func__, &cal_block->cal_data.paddr,
		adm_params.payload_size);
	...
	if (result < 0) {
		...
		ad_logd("%s: Set params failed port = 0x%x payload = 0x%pK result %d\n",
	...
	if (!result) {
		ad_loge("%s: Set params timed out port = 0x%x\n",
				__func__, port_id);
		ad_logd("%s: Set params timed out port = 0x%x, payload = 0x%pK\n",
	     ...
	     int perf_mode, uint16_t bit_width, int app_type, int acdb_id)
{
	...
	if (( topology == SRS_TRUMEDIA_TOPOLOGY_ID) && !perf_mode) {
		...
		(uint32_t *)&this_adm.outband_memmap.size, 1);
		if (res < 0) {
			ad_loge("%s: SRS adm_memory_map_regions failed ! addr = 0x%pK, size = %d\n",
...
int adm_map_rtac_block(struct rtac_cal_block_data *cal_block)
{
	...
	if (result < 0) {
		...
		ad_logd("%s: RTAC mmap did not work! addr = 0x%pK, size = %d\n",
			__func__,
			&cal_block->cal_data.paddr,
