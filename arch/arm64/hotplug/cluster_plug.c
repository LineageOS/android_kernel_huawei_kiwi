...
static void __ref cluster_plug_work_fn(struct work_struct *work)
{
	...
	if (cluster_plug_active) {
		...
		/* when a cpu doesn't have the expected state assume some other
		interuption (thermal throttling, ...) and do nothing */
		for_each_present_cpu(cpu) {
		...
		/* when disabling cluster_plug, bring back online the offlined cpus */
		if (offlined_cpus > 0) {
			plug_clusters(true);
			offlined_cpus = 0;
		}
